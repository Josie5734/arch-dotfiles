...
--current task: fix rules, in situation with a single cell in currentgen, a new cell is only created for (0,0,1), not for (0,1,0) or (1,0,0)
--next task: draw the rule diagrams and highlight the current one
		i think to do that, rules function should return another variable that is just the number of the current rule
		like in rules, have local rule = null and then all the rules have "rule = xrule" and then return rule
]]
...
function rules()
	...
	if ct[ci] == 1 then --rules 7,6,5,4 
		...
		if ct[ci+1] == 1 and ct[ci+2] == 1 then nt[ni] = 0 r = 7 --rule 7 (1,1,1) = 0
		elseif ct[ci+1] == 1 and ct[ci+2] == 0 then nt[ni] = 0 r = 6 --rule 6 (1,1,0) = 0
		elseif ct[ci+1] == 0 and ct[ci+2] == 1 then nt[ni] = 0 r = 5 --rule 5 (1,0,1) = 0
		elseif ct[ci+1] == 0 and ct[ci+2] == 0 then nt[ni] = 1 r = 4 --rule 4 (1,0,0) = 1
		end
		
	elseif ct[ci] == 0 then --rules 3,2,1,0

		if ct[ci+1] == 1 and ct[ci+2] == 1 then nt[ni] = 1 r = 3 --rule 3 (0,1,1) = 1
		elseif ct[ci+1] == 1 and ct[ci+2] == 0 then nt[ni] = 0 r = 2 --rule 2 (0,1,0) = 1
		elseif ct[ci+1] == 0 and ct[ci+2] == 1 then nt[ni] = 1 r = 1 --rule 1 (0,0,1) = 1
		elseif ct[ci+1] == 0 and ct[ci+2] == 0 then nt[ni] = 0 r = 0 --rule 0 (0,0,0) = 0
		end
