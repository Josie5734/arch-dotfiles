...
def password_output(homescreen, customers, clear):
    ...
    password = str(getpass()) # get password input as string, user input is hidden
    ...
    if password == "reset":
        ...
        password_output(homescreen, customers, clear) # reload password menu
    ...
    if valid == False: # if password was incorrect
        print("password incorrect : any - retry : q - quit")
        option = input()
   
        if option == "q": # if quit selected
            homescreen() # go back to homescreen
        else: password_output(homescreen,customers, clear) # if anything else selected, restart password function to retry
