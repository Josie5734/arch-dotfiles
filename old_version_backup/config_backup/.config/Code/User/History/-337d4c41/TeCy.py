    ...
    for row,value in enumerate(records): # for every row in records
        if row < max: # if not at the end of records
            ...
            while l < maxlets:
                ...
                if ord(records[row][l].upper()) < ord(records[row+1][l].upper()): # else if current letter is less than next
            ...
            #     if ord(records[row][letter].upper()) < ord(records[row+1][letter].upper()): # else if current letter is less than next
            #         break # exit loop, no need to check the rest
