...
def bubblesort_str_row(records, column, letter):
    ...
    for row,value in enumerate(records):
        if row < max:
            if ord(records[row][letter].upper()) > ord(records[row+1][letter].upper()): # if current letter of current word is bigger than next
                temp = records[row]
                records[row] = records[row+1]
                records[row+1] = temp
                compared = True

            elif ord(records[row][letter].upper()) == ord(records[row+1][letter].upper()): # if current letter is same as next
                bubblesort_str_row(records,column,letter+1)
            
    if compared == True:
        bubblesort_str_row(records,column,letter)
...
with open("words.txt","r") as file:
    for f in file:
        records.append(file.readline()[:-1])

bubblesort_str_row(records,2,0)