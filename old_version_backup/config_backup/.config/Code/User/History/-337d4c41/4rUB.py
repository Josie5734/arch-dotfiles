...
def bubblesort_str_row(records, column):
    ...
    for row,value in enumerate(records):
        if row < max:
            ...
            if len(records[row+1]) < maxlets: # check that if the next word has less letters
                maxlets = len(records[row+1]) # if so, make the max count the smaller number

            for l,v in enumerate(maxlets): # for every letter in the word (or upto the length of the next word if smaller)
