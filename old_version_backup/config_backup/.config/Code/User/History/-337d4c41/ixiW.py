...
def bubblesort_str_row(records, column):
    ...
    for row,value in enumerate(records): # for every row in records
        if row < max: # if not at the end of records
            maxlets = len(records[row][column]) # get the number of letters in the current word
            if len(records[row+1][column]) < maxlets: # check if the next word has less letters
                maxlets = len(records[row+1][column]) # if so, make the max count the smaller number

            l = 0 # index
            while l < maxlets: # while index is less than maxletters (for every letter in word)
                if ord(records[row][column][l].upper()) > ord(records[row+1][column][l].upper()): # if current letter of current word is bigger than next
                    temp = records[row][column] # store current
                    records[row][column] = records[row+1][column] # put next into current slot
                    records[row+1][column] = temp # put current into next slot
                    compared = True # mark compared
                    break # exit out of looping through each letter

                if ord(records[row][column][l].upper()) < ord(records[row+1][column][l].upper()): # else if current letter is less than next
                    break # exit loop, no need to check the rest

                # if not broken out by now, current letter must be same in current and next word
                l+=1 # iterate to next letter and go through loop again
