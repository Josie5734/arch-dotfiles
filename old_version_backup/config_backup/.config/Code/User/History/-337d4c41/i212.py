...
#TODO potentially: comparisons like "use" and "useless" end up with "use" put 2nd
    # not inherently an issue i guess but it seems counterintuitive
    # also words that are 3 letters long means the max can only be 2
    ...
    for i,word in enumerate(data): # for each item
        if i < max-1: # when not at end
            ...
            def compare(item1, item2, index):
                ...
                if ord(item1[index]) == ord(item2[index]) and index < max: # if letters in item1 and 2 are equal
