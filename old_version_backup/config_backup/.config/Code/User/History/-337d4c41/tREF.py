...
def bubblesort_str_row(records, column, letter):
    ...
    for row,value in enumerate(records):
        if row < max:
            if ord(records[row][letter].upper()) > ord(records[column][letter].upper()): # if current letter of current word is bigger than next
                print("do swap")

            elif ord(records[row][letter].upper()) == ord(records[column][letter].upper()): # if current letter is same as next
                print("do again but shift the letter to use")
