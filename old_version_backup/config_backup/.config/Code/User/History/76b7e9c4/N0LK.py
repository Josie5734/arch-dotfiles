...
# reload the current menu, just easier to read
def reload(table,extra = None):
    exec("{}({})".format(table,extra)) # dynamically reload menu, assuming table is the same as the menu function name
...
# options for the menu, takes user input and executes functions
def menu_choice(input,table,records,headers):
    match input:
        case "1": # edit an entry
            print("enter {}ID to edit : q to quit".format(table)) # get primary key of row to edit
            key = str(input()) # store as string

            if key == "q": # if input is q for quit
                reload(table) # reload menu
        ...
        case "2": # add an entry
            ...
            for h in headers[1:]: # for each header in the current table, starting from the one after the ID
                ...
                newval = input() # take the user input as string
                newval = str(newval)
        ...
        case "3": # remove an entry
            print("enter {}ID to remove : q to quit".format(table)) # get id to remove
            key = str(input()) # store it

            if key == "q": # if q for quit
                reload(table) # reload menu without doing anything
                ...
                reload(table) # dynamically reload menu, assuming table is the same as the menu function name
        ...
        case "4": # sort
            ...
            reload(table,sort_recs) # reload menu with sorted data
        ...
        case "5": # filter
            ...
            reload(table,filter_recs) # reload menu with filtered data
...
def employees(records = None): 
    ...
    # do stuff with input
    menu_choice(option,table,records,headers)
...
def customers(valid = False):
    ...
    if valid == False: # if correct password has not been given
        ...
        print("customers menu not currently implemented \n press any button to continue")
