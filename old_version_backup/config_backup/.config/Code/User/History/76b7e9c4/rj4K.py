...
def get_headers(table):
    ...
    cursor.execute("SELECT * FROM {}".format(table)) # read table 
...
def employees(records = None): 
    ...
    # get data pieces
    headers = get_headers(table,cursor,connection) # get headers of table
    
    if records == None: # if no record has been passed in
        records = get_data(table,cursor,connection) # get the data for the current table
