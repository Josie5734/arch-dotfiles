...
### current objective:
    ...
    # currently edit_db.py is empty, can be deleted
    # create_db.py can be renamed to just database.py or something general
    # functions currently in create_db.py will be reasssessed for usefulness
    # db functions in main.py will be moved to new database.py
    # update stuff in main.py to work with moved functions

    # additionally, adjust functions to use the currently unused modular db connect and close functions once everything is refactored


# look into SQL increment column for the primary keys - might be more complicated than just letting the user input it themselves
    # it just sets the primary key to increment by one for each new row, not sure how it handles deletions
    # could be useful for clean ordering, but also would mean reimplementing add/edit/remove functions to ignore the primary key 

# primary as part of the prototype phase:

# may be good to de-globalise the db cursor and connection, atleast into within the menus
    # currently have functions to open and close a connection, just need to decide on a way to implement them

# figure out a system for using the try catch finally thingy + logging errors with custom log.py functions
...
def homescreen():
    ...
    match option: # switch case for input
        ...
        case "8":
            myrecs = dummydata.dummy_employee(100) # generate data  -  for some reason cannot find the function
            headers = db_functions.get_headers("employees")
            for r in myrecs:
                db_functions.add_row("employees",cursor,connection,r,headers)
            employees()

        case "9":
            db_functions.create_full_db()
...
def employees(records = None): 
    ...
    table = "employees" # holds the table name for passing to functions so it isnt hardcoded for every function call

    # get data pieces
    headers = db_functions.get_headers(table) # get headers of table
    
    if records == None: # if no record has been passed in
        records = db_functions.get_data(table) # get the data for the current table

    def output(data_table): # default output for the menu
        print("employees \n")
    ...
    match option:
        ...
        case "4": # sort
            ...
            employees(sort_recs) # update menu, passing in sorted records to use as output
