...
def password_file(password, operation):
    ...
    if operation == "w": # writing new password
        ...
        with open("pass.txt") as file: # open file
...
def password_output(homescreen, customers):
    ...
    if password == "reset":
        ...
        password_output() # reload password menu
    ...
    if valid == False: # if password was incorrect
        ...
        if input == "r": # if retry selected
            password_output() # restart password function
        elif input == "q": # if quit selected
